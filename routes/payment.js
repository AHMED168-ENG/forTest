...
// *********** send mony to user ********************//
router.post('/send-mony-to-user/:user_id' , sendMonyToUser() , handel_validation_errors , async (req, res, next) => {
    try {
        let {amount} = req.body
        ...
        const userWalletAfter = await wallet_model.findOneAndUpdate({user_id : user_id} , {user_wallet : {$inc : amount}} , {new : true})
...
// *********** get mony balance ********************//
router.post('/get-user-balance/:user_id' , sendMonyToUser() , handel_validation_errors , async (req, res, next) => {
    try {
        let {user_id} = req.params
        const appManager = await app_manager_model.findOne({})
        const userWallet = await wallet_model.findOne({user_id : req.user.id})
        ...
        const userWalletAfter = await wallet_model.findOneAndUpdate({user_id : user_id} , {user_wallet : {$inc : 1000} , balance : 0} , {new : true})
        return userWalletAfter
    } catch (e) {
        next(e)
    }
